    ...
    while (!$products_new->EOF) {
      ...
      if (PRODUCT_NEW_LIST_IMAGE != '0') {
        if ($products_new->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) {
          ...
          $display_products_image = '<a href="/index.php?main_page=product_info&products_id='.$products_new->fields['products_id'].'">' . zen_image(DIR_WS_IMAGES . $products_new->fields['products_image'], $products_new->fields['products_name'], IMAGE_PRODUCT_NEW_LISTING_WIDTH, IMAGE_PRODUCT_NEW_LISTING_HEIGHT) . '</a>' . str_repeat('<br clear="all" />', substr(PRODUCT_NEW_LIST_IMAGE, 3, 1));
        }
      } else {
      ...
      if (PRODUCT_NEW_LIST_NAME != '0') {
        $display_products_name = '<a href="/index.php?main_page=product_info&products_id='.$products_new->fields['products_id'].'"><strong>' . $products_new->fields['products_name'] . '</strong></a>' . str_repeat('<br clear="all" />', substr(PRODUCT_NEW_LIST_NAME, 3, 1));
      } else {
...
// more info in place of buy now
      if (PRODUCT_NEW_BUY_NOW != '0' and zen_get_products_allow_add_to_cart($products_new->fields['products_id']) == 'Y') {
        if (zen_has_product_attributes($products_new->fields['products_id'])) {
          //$link = '<a href="/index.php?main_page=product_info&products_id='.$products_new->fields['products_id'].'">' . MORE_INFO_TEXT . '</a>';
        } else {
...
//            $how_many++;
        ...
        $the_button = $link;
        //$products_link = '<a href="/index.php?main_page=product_info&products_id='.$products_new->fields['products_id'].'">' . MORE_INFO_TEXT . '</a>';
      ...
      if (PRODUCT_NEW_LIST_DESCRIPTION > '0') {
        ...
        //$display_products_description = stripslashes(zen_trunc_string($disp_text, PRODUCT_NEW_LIST_DESCRIPTION, '<a href="/index.php?main_page=product_info&products_id='.$products_new->fields['products_id'].'"> ' . MORE_INFO_TEXT . '</a>'));
      } else {
...
?>
          <tr>
            <td width="<?php echo IMAGE_PRODUCT_NEW_LISTING_WIDTH + 10; ?>" valign="top" class="main newproduct-mainleft" align="center">
              <?php
            ...
            <td class="mj-newproductprice">
            <?php
			$disp_sort_order1 = $db->Execute("select configuration_key, configuration_value from " . TABLE_CONFIGURATION . " where configuration_group_id='" . $group_id . "' and (configuration_value >= 1000 and configuration_value <= 1999) order by LPAD(configuration_value,11,0)");
			
			
			 while (!$disp_sort_order1->EOF) {
				  
				  if ($disp_sort_order1->fields['configuration_key'] == 'PRODUCT_NEW_BUY_NOW') {
                    ?><div class="mj-productnewprice"><?php echo $display_products_button; ?></div><?php
                  }
				  if ($disp_sort_order1->fields['configuration_key'] == 'PRODUCT_NEW_LIST_QUANTITY') {
                   ?><div class="mj-productnewprice"><?php echo $display_products_quantity; ?></div> <?php
                  }
            ...
            <td colspan="2" valign="top" class="main newproduct-mainright">
              <?php
			...
			$disp_sort_order1 = $db->Execute("select configuration_key, configuration_value from " . TABLE_CONFIGURATION . " where configuration_group_id='" . $group_id . "' and (configuration_value >= 2000 and configuration_value <= 2999) order by LPAD(configuration_value,11,0)");
			 ...
			 while (!$disp_sort_order1->EOF) {
				  ...
				  if ($disp_sort_order1->fields['configuration_key'] == 'PRODUCT_NEW_LIST_PRICE') {
                    ?><div class="mj-productnewprice"><?php echo $display_products_price; ?></div><?php
                  }
                  $disp_sort_order1->MoveNext();
				   }
			?>
...
<?php } ?>
          ...
          <tr>
            <td colspan="3" class="mj-productborder"></td>
          </tr>
<?php
