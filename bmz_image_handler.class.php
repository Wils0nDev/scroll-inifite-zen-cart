...
require_once('functions_bmz_io.php');
require_once('bmz_io_conf.php');
require_once('bmz_image_handler_conf.php');
class ih_image{
 ...
 */
    ...
    $this->filename =  "http://localhost/apiphp/".$this->src;
  ...
  function resize_imageGD($file_ext, $dest_name, $bg, $quality = 85) {
    ...
    if ($this->sizetype == 'small') {
      if ($ihConf[$this->sizetype]['zoom']) {
        ...
        return $parameters . 'style="position:relative;max-height:100% !important; max-width:100% !important;" onmouseover="showtrail(' . "'$products_image_zoom','$alt',$width,$height,$zoomwidth,$zoomheight,this," . $this->zoom['startx'].','.$this->zoom['starty'].','.$this->zoom['width'].','.$this->zoom['height'].');" onmouseout="hidetrail();" ';
      }
       return $parameters;
    }
    return $parameters;
